...
coef_path = get_model_coef_file_path(L2, only_nonprod_sites = FALSE)
...
# plot mh heatmap
mh_heatmap = plot_mh_coefficient_heatmap_single_group(coefs, with_values = FALSE, limits = c(-0.435, 0.435), prop = FALSE, positions = c('mid'))
...
together = plot_grid(first_grid, NULL, second_grid, NULL, third_grid, NULL, legend, nrow = 7, rel_heights = c(1, 0.08, 1, 0.08, 1.4, 0.08, 0.2))

# save plot
file_name = paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/motif_base_count_interior-mh-count_model/coef_heatmap.pdf')

ggsave(file_name, plot = together, width = 18, height = 23, units = 'in', dpi = 750, device = cairo_pdf)
