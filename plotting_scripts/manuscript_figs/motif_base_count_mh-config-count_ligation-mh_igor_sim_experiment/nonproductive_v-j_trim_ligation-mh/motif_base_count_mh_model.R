...
fwrite(coefs, paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/motif_base_count_mh-config-count_ligation-mh_igor_sim_experiment/', PARAM_GROUP, '/coefs.tsv'), sep = '\t')

v_motif_heatmap = plot_motif_coefficient_heatmap_single_group(coefs[trim_type == 'v_trim'], with_values = FALSE, limits = c(-1.072, 1.072)) + ggtitle('   V-trimming coefficients')
j_motif_heatmap = plot_motif_coefficient_heatmap_single_group(coefs[trim_type == 'j_trim'], with_values = FALSE, limits = c(-1.072, 1.072)) + ggtitle('   J-trimming coefficients')
...
v_base_count_heatmap = plot_base_count_coefficient_heatmap_single_group(coefs[trim_type == 'v_trim'], with_values = FALSE, limits = c(-1.072, 1.072))
j_base_count_heatmap = plot_base_count_coefficient_heatmap_single_group(coefs[trim_type == 'j_trim'], with_values = FALSE, limits = c(-1.072, 1.072))
...
# plot mh heatmap
mh_heatmap = plot_ligation_mh_coefficient_heatmap_single_group(coefs, with_values = FALSE, limits = c(-1.072, 1.072))
config_heatmap = plot_mh_config_count_coefficient_heatmap_single_group(coefs, with_values = FALSE, limits = c(-1.072, 1.072))
...
# save plot
file_name = paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/motif_base_count_mh-config-count_ligation-mh_igor_sim_experiment/', PARAM_GROUP, '/coef_heatmap.pdf')
