...
file_root = paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/motif_base_count_average-mh_ligation-mh_model/', PARAM_GROUP, '/standardized_coefs')

# Read in model coefficient data 
coef_path = get_model_coef_file_path(L2)
coef_path = str_replace(coef_path, 'mh_ligation-mh', 'mh_ligation-mh/standardized_coefs')
...
bar = ggplot(coefs, aes(x = clean_name, y = abs(value)/log(10), fill = type)) +
