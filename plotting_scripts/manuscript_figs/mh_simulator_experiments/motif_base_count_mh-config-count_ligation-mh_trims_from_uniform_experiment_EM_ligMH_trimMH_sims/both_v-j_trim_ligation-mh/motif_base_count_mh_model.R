...
for (trim_model in TRIMMING_PROB_MODEL){
    for (param in LIGATION_PARAMS){
        for (param2 in INT_MH_PARAMS){
            ...
            fwrite(coefs, paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/motif_base_count_mh-config-count_ligation-mh_trims_from_uniform_experiment_EM_ligMH_trimMH_sims/', PARAM_GROUP, '/coefs_MHprob', param, '_', trim_model, '_trim-model.tsv'), sep = '\t')
    ...
    # save plot
    file_name = paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/motif_base_count_mh-config-count_ligation-mh_trims_from_uniform_experiment_EM_ligMH_trimMH_sims/', PARAM_GROUP, '/coef_heatmap_', trim_model, '_trim-model.pdf')
