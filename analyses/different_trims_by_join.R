...
if (NT_COUNT > 5){
    ...
    file_name = paste0(path, '/', LOCUS, '_', TRIM_TYPE, '_pairwise_SAD_versus_hamming_aligned_', NT_COUNT, '_nt_', JOINING_GENE, '.pdf')
...
file_name = paste0(path, '/', LOCUS, '_', TRIM_TYPE, '_pairwise_SAD_versus_alignment_', NT_COUNT, '_nt_', JOINING_GENE, '.pdf')
...
cluster_dists_hamming = plot_general_boxplot(cluster_hamming[!(j_gene.x == j_gene.y)], xvar = 'cluster', yvar = 'dist', xtitle = '\nTrimming distribution cluster (from K-means)', ytitle = paste0('Pairwise hamming distance (first ', NT_COUNT, ' nt)\n'), title = '', facet_var = 'v_gene', facet_col = 5)
 
file_name = paste0(path, '/', LOCUS, '_', TRIM_TYPE, '_cluster_versus_hamming_', NT_COUNT, '_nt_', JOINING_GENE, '.pdf')
...
cluster_dists_align = plot_general_boxplot(cluster_align[!(j_gene.x == j_gene.y)], xvar = 'cluster', yvar = 'pairwise_score', xtitle = '\nTrimming distribution cluster (from K-means)', ytitle = paste0('Pairwise global alignment score (first ', NT_COUNT, ' nt)\n'), title = '', facet_var = 'v_gene', facet_col = 5)
 
file_name = paste0(path, '/', LOCUS, '_', TRIM_TYPE, '_cluster_versus_align_', NT_COUNT, '_nt_', JOINING_GENE, '.pdf')
