...
IGOR_MARG_PATH=sys.argv[1]
IGOR_PARAM_PATH=sys.argv[2]
MOD_OUTPUT_PATH=sys.argv[3]
print(MOD_OUTPUT_PATH)

mdl_hb = p3.get_default_IgorModel("human", "tcr_alpha")
mdl_hb = p3.IgorModel(model_parms_file=IGOR_PARAM_PATH, model_marginals_file=IGOR_MARG_PATH)
vgenes = pd.DataFrame()
...
path=MOD_OUTPUT_PATH + '/igor_alpha_j_trim_params.tsv'
jgenes.to_csv(path, sep='\t', index=False)

path=MOD_OUTPUT_PATH + '/igor_alpha_v_trim_params.tsv'
...
path=MOD_OUTPUT_PATH + '/igor_alpha_vchoice_params.tsv'
...
path=MOD_OUTPUT_PATH + '/igor_alpha_jchoice_params.tsv'
