#!/bin/bash

set -eu

RAW_FILE_PATH=$1
TEMP_DIR=$2
ADAPTIVE=$3
LOCUS=$4
OUTPUT_DIR=$5
ANNOTATION_COUNT=$6
MODEL_PARAMS=$7
MODEL_MARG=$8
NCPU=$9
PARTITION=${10}

mkdir -p $TEMP_DIR
mkdir -p $OUTPUT_DIR

for RAW_FILE in $RAW_FILE_PATH/*.tsv; do
    TEMP_OUTPUT_FILE=$(basename -s .tsv $RAW_FILE)
    TEMP_OUTPUT_FILE="$TEMP_DIR/$TEMP_OUTPUT_FILE"
    mkdir $TEMP_OUTPUT_FILE
    TEMP_OUTPUT_FILE="$TEMP_OUTPUT_FILE/running" 

    COMMAND="sbatch -c $NCPU -p $PARTITION -q $PARTITION scripts/igor_scripts/annotate/run_igor_annotation_all.sh $RAW_FILE $TEMP_DIR $ADAPTIVE $LOCUS $OUTPUT_DIR $MODEL_PARAMS $MODEL_MARG $ANNOTATION_COUNT $NCPU"
    echo $COMMAND

    $COMMAND > $TEMP_OUTPUT_FILE
done
