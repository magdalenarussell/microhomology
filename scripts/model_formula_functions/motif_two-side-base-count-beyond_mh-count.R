...
get_parameter_vector <- function(trims, genes){
    ...
    mh_vars = get_all_mh_count_variables(overlap_vector = c(0, 1, 2, 3, 4))
...
process_single_data_for_model_fit <- function(group_motif_data, whole_nucseq = get_oriented_whole_nucseqs(), gene_type = GENE_NAME, trim_type = TRIM_TYPE){
    ...
    together = process_for_mh(together, whole_nucseq = whole_nucseq, overlap_vector = c(0, 1, 2, 3, 4), trim_type = trim_type, gene_type = gene_type, prop = FALSE)
    return(together)
}
